...
# ── Existing config ─────────────────────────────────────────────────────────
MSG_CONFIG_EXISTS_REUSE="Найдена существующая конфигурация. Использовать её?"
...
MSG_CONFIG_RESTART_CONFIRM="Перезапустить службу для применения изменений?"
MSG_CONFIG_RESTART_SKIPPED="Перезапуск отменён. Изменения применятся при следующем перезапуске."
